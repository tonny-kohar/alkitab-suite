...
public class SwingHTMLConverter implements Converter {
    ...
    public SAXEventProvider convert(SAXEventProvider xmlsep) throws TransformerException {
        try {
            ...
            /* XXX: workaround for Java 7 and xalan use of extension function 'http://xml.apache.org/xalan/java:instance'
             * is not allowed when the secure processing feature is set to true.
             * By using reflection
             * - Create new TransformerFactory
             * - set isNotSecureProcessing to true 
             * - Replace the TransformingSAXEventProvider.transfact with the above 
             */
            try {
                TransformerFactory tf = TransformerFactory.newInstance();
                Field _isNotSecureProcessing = tf.getClass().getDeclaredField("_isNotSecureProcessing");
                _isNotSecureProcessing.setAccessible(true);
                _isNotSecureProcessing.set(tf, Boolean.TRUE);
                
                Field _transfact = tsep.getClass().getDeclaredField("transfact");
                _transfact.setAccessible(true);
